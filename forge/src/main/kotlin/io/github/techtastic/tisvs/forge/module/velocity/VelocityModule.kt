package io.github.techtastic.tisvs.forge.module.velocity
...
class VelocityModule(casing: Casing, face: Face): AbstractModuleWithRotation(casing, face) {
    ...
    override fun render(context: RenderContext) {
        ...
        context.drawAtlasQuadUnlit(ResourceLocation(MOD_ID,
            if (context.closeEnoughForDetails(casing.position))
                "block/overlay/velocity/${output.toString().lowercase()}"
            else
                "item/velocity_module"
        ))
        ...
        if (context.closeEnoughForDetails(casing.position)) {
            ...
            val width = font.width(velocity).toDouble()
            poseStack.translate(32 - width / 2, 10.0, 0.0)

            // Draw String
            context.drawString(font, velocity, Color.WHITE)
        }
